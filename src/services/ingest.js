...
/** Embed an array of texts in batches with the configured embedding model. */
...
/**
 * Ingest every supported document in a directory into the local Chroma
 * collection.
 *
 ...
 * calls.
 *
 * @param {string} docsDir directory containing source documents
 * @returns {Promise<{ files: number, chunks: number, embedded: number, skipped: number }>}
 */
export async function ingestDocs(docsDir) {
